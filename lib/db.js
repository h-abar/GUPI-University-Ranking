...
async function ensureSchema() {
  ...
  const { rows: aRows } = await p.query('SELECT COUNT(*) as c FROM admins');
  if (parseInt(aRows[0].c) === 0) {
    const hash = bcrypt.hashSync('admin123', 10);
    await p.query('INSERT INTO admins (username, password_hash) VALUES ($1, $2)', ['admin', hash]);
  }
