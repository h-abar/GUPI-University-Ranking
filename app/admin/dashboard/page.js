...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="min-h-screen bg-gupi-ink-50">
      {/* Admin header */}
      <div className="bg-gupi-orange-950 text-white py-4 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-gupi-orange-400 to-gupi-orange-600 flex items-center justify-center font-bold">G</div>
            <div>
              <div className="font-display font-bold">GUPI Admin</div>
              <div className="text-xs text-gupi-ink-400">لوحة التحكم الشاملة</div>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <a href="/" className="text-sm text-gupi-ink-300 hover:text-white">عرض الموقع</a>
      ...
      {/* Tabs */}
      <div className="bg-white border-b border-gupi-ink-200 sticky top-[65px] z-30">
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-5 py-4 text-sm font-medium border-b-2 transition-colors whitespace-nowrap ${
                    tab === t.key
                      ? 'border-gupi-orange-600 text-gupi-orange-700'
                      : 'border-transparent text-gupi-ink-500 hover:text-gupi-ink-700 hover:bg-gupi-ink-50'
                  }`}
                >
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'إجمالي الجامعات', value: universities.length, icon: Database, color: 'from-gupi-orange-500 to-gupi-orange-700' },
          { label: 'الدول الممثلة', value: countries.size, icon: Globe, color: 'from-emerald-500 to-emerald-700' },
          { label: 'متوسط GUPI', value: avgScore, icon: Award, color: 'from-gupi-amber-500 to-gupi-amber-700' },
          ...
          return (
            <div key={i} className="bg-white rounded-2xl shadow p-6">
              ...
              <div className="text-2xl font-black text-gupi-ink-800">{s.value}</div>
              <div className="text-sm text-gupi-ink-500 mt-1">{s.label}</div>
            </div>
          );
        })}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white rounded-2xl shadow p-6">
          <div className="text-sm text-gupi-ink-500 mb-1">أعلى درجة</div>
          <div className="text-3xl font-black text-green-600">{topScore}</div>
        </div>
        <div className="bg-white rounded-2xl shadow p-6">
          <div className="text-sm text-gupi-ink-500 mb-1">أدنى درجة</div>
      ...
      {/* Score distribution */}
      <div className="bg-white rounded-2xl shadow p-6">
        <h3 className="font-display font-bold text-lg text-gupi-orange-900 mb-4">توزيع الدرجات</h3>
        <div className="space-y-3">
          {Object.entries(scoreBuckets).map(([range, count]) => (
            <div key={range} className="flex items-center gap-4">
              <div className="w-16 text-sm font-medium text-gupi-ink-600">{range}</div>
              <div className="flex-1 bg-gupi-ink-100 rounded-full h-8 overflow-hidden">
                <div
                  className="h-full bg-gradient-to-r from-gupi-orange-400 to-gupi-orange-600 rounded-full flex items-center justify-end px-3"
      ...
      <div className="bg-white rounded-2xl shadow overflow-hidden">
        <div className="p-6 border-b border-gupi-ink-100">
          <h3 className="font-display font-bold text-lg text-gupi-orange-900">أفضل 10 جامعات</h3>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {universities.slice(0, 10).map((u) => (
                <tr key={u.id} className="border-b border-gupi-ink-50">
                  <td className="px-4 py-3">
                    <div className={`rank-badge w-8 h-8 text-sm ${
                      u.rank === 1 ? 'rank-1' : u.rank === 2 ? 'rank-2' : u.rank === 3 ? 'rank-3' : 'rank-other'
                    }`}>{u.rank}</div>
                  </td>
                  <td className="px-4 py-3 font-medium text-gupi-orange-900">{u.name}</td>
                  <td className="px-4 py-3 text-center text-gupi-ink-600">{u.country}</td>
                  <td className="px-4 py-3 text-center font-bold text-gupi-orange-700">{u.gupi.totalScore}</td>
                  <td className="px-4 py-3 text-center text-gupi-orange-600">{u.gupi.presenceScore}/{u.gupi.maxPresence}</td>
                  <td className="px-4 py-3 text-center text-gupi-amber-600">{u.gupi.excellenceScore}/{u.gupi.maxExcellence}</td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow p-6">
        <div className="flex items-center gap-4 flex-wrap">
          <label className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gupi-orange-600 text-white font-medium cursor-pointer hover:bg-gupi-orange-700 text-sm">
          ...
          {importStatus && (
            <div className={`flex items-center gap-2 text-sm ${
              ...
              importStatus.type === 'error' ? 'text-red-600' : 'text-gupi-orange-600'
            }`}>
      ...
      <div className="bg-white rounded-2xl shadow overflow-hidden">
        <div className="p-6 border-b border-gupi-ink-100">
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <h2 className="font-display font-bold text-lg text-gupi-orange-900">إدارة الجامعات ({filtered.length})</h2>
            <div className="flex gap-3 flex-wrap">
              <div className="relative">
                <Search className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gupi-ink-400" />
                <input type="text" placeholder="بحث..." value={search} onChange={(e) => setSearch(e.target.value)}
                  className="pr-9 pl-3 py-2 rounded-lg border border-gupi-ink-200 text-sm focus:border-gupi-orange-500 outline-none w-40" />
              </div>
              <select value={countryFilter} onChange={(e) => setCountryFilter(e.target.value)}
                className="px-3 py-2 rounded-lg border border-gupi-ink-200 text-sm focus:border-gupi-orange-500 outline-none bg-white">
                <option value="all">كل الدول</option>
                {countries.map((c) => <option key={c} value={c}>{c}</option>)}
              </select>
              <button onClick={handleAdd} className="flex items-center gap-1 px-4 py-2 rounded-lg bg-gupi-orange-600 text-white text-sm font-medium hover:bg-gupi-orange-700">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((u) => (
                <tr key={u.id} className="border-b border-gupi-ink-50 hover:bg-gupi-ink-50/50">
                  <td className="px-4 py-3 text-gupi-ink-400">{u.rank}</td>
                  <td className="px-4 py-3 font-medium text-gupi-orange-900">{u.name}</td>
                  <td className="px-4 py-3 text-center text-gupi-ink-600">{u.country}</td>
                  <td className="px-4 py-3 text-center font-bold text-gupi-orange-700">{u.gupi.totalScore}</td>
                  <td className="px-4 py-3 text-center text-gupi-orange-600">{u.gupi.presenceScore}/{u.gupi.maxPresence}</td>
                  <td className="px-4 py-3 text-center text-gupi-amber-600">{u.gupi.excellenceScore}/{u.gupi.maxExcellence}</td>
                  <td className="px-4 py-3 text-center text-gupi-ink-600">{u.articles_2025?.toLocaleString() || '—'}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-center gap-2">
                      <button onClick={() => handleEdit(u)} className="p-2 rounded-lg text-gupi-orange-600 hover:bg-gupi-orange-50"><Edit2 className="w-4 h-4" /></button>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={() => setShowForm(false)}>
          <div className="bg-white rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="sticky top-0 bg-white border-b border-gupi-ink-100 p-6 flex items-center justify-between z-10">
              <h2 className="font-display font-bold text-xl text-gupi-orange-900">{editingId ? 'تعديل جامعة' : 'إضافة جامعة جديدة'}</h2>
              <button onClick={() => setShowForm(false)} className="p-2 rounded-lg hover:bg-gupi-ink-100"><X className="w-5 h-5" /></button>
            </div>

            {/* Live score preview */}
            {liveScore && (
              <div className="bg-gradient-to-l from-gupi-orange-50 to-gupi-amber-50 p-4 border-b border-gupi-ink-100">
                <div className="flex items-center gap-4 justify-center">
                  <div className="text-center">
                    <div className="text-xs text-gupi-ink-500">معاينة الدرجة</div>
                    <div className="text-3xl font-black text-gupi-orange-700">{liveScore.total}</div>
                  </div>
                  <div className="text-center">
                    <div className="text-xs text-gupi-ink-500">حضور</div>
                    <div className="text-xl font-bold text-gupi-orange-600">{liveScore.p}</div>
                  </div>
                  <div className="text-center">
                    <div className="text-xs text-gupi-ink-500">تميز</div>
                    <div className="text-xl font-bold text-gupi-amber-600">{liveScore.e}</div>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gupi-ink-700 mb-1">سنة التأسيس</label>
                  <input type="number" value={formData.founded} onChange={(e) => setFormData({ ...formData, founded: e.target.value })}
                    className="w-full px-3 py-2 rounded-lg border border-gupi-ink-200 focus:border-gupi-orange-500 outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gupi-ink-700 mb-1">عدد الأبحاث 2025</label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="md:col-span-1">
                  <label className="block text-sm font-medium text-gupi-ink-700 mb-1">الرمز المختصر</label>
                  <input type="text" value={formData.short_code} onChange={(e) => setFormData({ ...formData, short_code: e.target.value })}
                    placeholder="مثال: KSU" className="w-full px-3 py-2 rounded-lg border border-gupi-ink-200 focus:border-gupi-orange-500 outline-none" />
                </div>
                <div className="md:col-span-3">
                  <label className="block text-sm font-medium text-gupi-ink-700 mb-1">وصف الجامعة (يظهر في بطاقة الجامعة)</label>
                  <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                    rows={3} placeholder="وصف مختصر عن الجامعة..."
                    className="w-full px-3 py-2 rounded-lg border border-gupi-ink-200 focus:border-gupi-orange-500 outline-none resize-none" />
                </div>
              </div>

              <div>
                <h3 className="font-bold text-gupi-ink-700 mb-3">التصنيفات</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {ALL_FIELDS.map((field) => {
                    const cfg = configs.find((c) => c.field_key === field.key);
                    return (
                      <div key={field.key}>
                        <label className="block text-xs font-medium text-gupi-ink-600 mb-1 flex items-center gap-1">
                          {field.label}
                          {cfg?.is_presence && <span className="text-[9px] bg-gupi-orange-100 text-gupi-orange-700 px-1 rounded">حضور</span>}
                          {cfg?.is_excellence && <span className="text-[9px] bg-gupi-amber-100 text-gupi-amber-700 px-1 rounded">تميز</span>}
                        </label>
                        <input type="text" value={formData[field.key]} onChange={(e) => setFormData({ ...formData, [field.key]: e.target.value })}
                          placeholder="—" className="w-full px-3 py-2 rounded-lg border border-gupi-ink-200 focus:border-gupi-orange-500 outline-none text-sm" />
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="flex gap-3 pt-4 border-t border-gupi-ink-100">
                <button onClick={handleSave} className="flex items-center gap-2 px-6 py-2.5 rounded-lg bg-gupi-orange-600 text-white font-medium hover:bg-gupi-orange-700">
                  <Save className="w-4 h-4" /> حفظ
                </button>
                <button onClick={() => setShowForm(false)} className="px-6 py-2.5 rounded-lg border border-gupi-ink-200 text-gupi-ink-600 font-medium hover:bg-gupi-ink-50">
                  إلغاء
                </button>
              </div>
            </div>
...
function RankingsTab({ configs, settings, onReload }) {
  ...
  return (
    <div className="space-y-6">
      {/* Info cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gradient-to-br from-gupi-orange-50 to-white rounded-2xl p-6 border border-gupi-orange-200">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-xl bg-gupi-orange-600 text-white flex items-center justify-center">
              <Globe className="w-6 h-6" />
            </div>
            <div>
              <div className="text-2xl font-black text-gupi-orange-700">{presenceCount}</div>
              <div className="text-sm text-gupi-ink-600">تصنيفات الحضور</div>
            </div>
          </div>
        </div>
        <div className="bg-gradient-to-br from-gupi-amber-50 to-white rounded-2xl p-6 border border-gupi-amber-200">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-xl bg-gupi-amber-500 text-white flex items-center justify-center">
              <Trophy className="w-6 h-6" />
            </div>
            <div>
              <div className="text-2xl font-black text-gupi-amber-700">{excellenceCount}</div>
              <div className="text-sm text-gupi-ink-600">تصنيفات التميز</div>
            </div>
          </div>
        </div>
        <div className="bg-gradient-to-br from-gupi-ink-50 to-white rounded-2xl p-6 border border-gupi-ink-200">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-xl bg-gupi-ink-600 text-white flex items-center justify-center">
              <Database className="w-6 h-6" />
            </div>
            <div>
              <div className="text-2xl font-black text-gupi-ink-700">{localConfigs.length}</div>
              <div className="text-sm text-gupi-ink-600">إجمالي التصنيفات</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <button onClick={() => setShowAdd(true)} className="flex items-center gap-1 px-4 py-2.5 rounded-xl bg-white border border-gupi-ink-200 text-gupi-ink-700 font-medium hover:bg-gupi-ink-50 mr-auto">
      ...
      <div className="bg-white rounded-2xl shadow overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {sorted.map((cfg) => (
                <tr key={cfg.id} className={`border-b border-gupi-ink-50 ${!cfg.active ? 'opacity-40' : ''}`}>
                  <td className="px-3 py-2">
                    <div className="flex items-center justify-center gap-1">
                      <button onClick={() => moveConfig(cfg.id, -1)} className="p-1 rounded hover:bg-gupi-ink-100"><ChevronUp className="w-3 h-3" /></button>
                      <button onClick={() => moveConfig(cfg.id, 1)} className="p-1 rounded hover:bg-gupi-ink-100"><ChevronDown className="w-3 h-3" /></button>
                    </div>
                  </td>
                  <td className="px-3 py-2">
                    <input type="text" value={cfg.label_ar} onChange={(e) => updateConfig(cfg.id, 'label_ar', e.target.value)}
                      className="w-full px-2 py-1 rounded border border-gupi-ink-200 text-sm focus:border-gupi-orange-500 outline-none" />
                  </td>
                  <td className="px-3 py-2">
                    <code className="text-xs text-gupi-ink-500">{cfg.field_key}</code>
                  </td>
                  <td className="px-3 py-2">
                    <select value={cfg.category} onChange={(e) => updateConfig(cfg.id, 'category', e.target.value)}
                      className="px-2 py-1 rounded border border-gupi-ink-200 text-xs focus:border-gupi-orange-500 outline-none bg-white">
                  ...
                  <td className="px-3 py-2 text-center">
                    <input type="checkbox" checked={!!cfg.is_presence} onChange={(e) => updateConfig(cfg.id, 'is_presence', e.target.checked ? 1 : 0)}
                      className="w-5 h-5 accent-gupi-orange-600 cursor-pointer" />
                  </td>
                  <td className="px-3 py-2 text-center">
                    <input type="number" step="0.1" min="0" value={cfg.presence_weight} onChange={(e) => updateConfig(cfg.id, 'presence_weight', parseFloat(e.target.value) || 0)}
                      className="w-16 px-2 py-1 rounded border border-gupi-ink-200 text-sm text-center focus:border-gupi-orange-500 outline-none" />
                  </td>
                  <td className="px-3 py-2 text-center">
                    <input type="checkbox" checked={!!cfg.is_excellence} onChange={(e) => updateConfig(cfg.id, 'is_excellence', e.target.checked ? 1 : 0)}
                      className="w-5 h-5 accent-gupi-amber-500 cursor-pointer" />
                  </td>
                  <td className="px-3 py-2 text-center">
                    <input type="number" step="0.1" min="0" value={cfg.excellence_weight} onChange={(e) => updateConfig(cfg.id, 'excellence_weight', parseFloat(e.target.value) || 0)}
                      className="w-16 px-2 py-1 rounded border border-gupi-ink-200 text-sm text-center focus:border-gupi-amber-500 outline-none" />
                  </td>
                  <td className="px-3 py-2 text-center">
                    <button onClick={() => updateConfig(cfg.id, 'active', cfg.active ? 0 : 1)}
                      className="p-1.5 rounded-lg hover:bg-gupi-ink-100">
                      {cfg.active ? <Eye className="w-4 h-4 text-green-500" /> : <EyeOff className="w-4 h-4 text-gupi-ink-400" />}
                    </button>
                  </td>
                  <td className="px-3 py-2 text-center">
      ...
      {showAdd && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={() => setShowAdd(false)}>
          <div className="bg-white rounded-2xl max-w-lg w-full" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 border-b border-gupi-ink-100 flex items-center justify-between">
              <h3 className="font-display font-bold text-lg text-gupi-orange-900">إضافة تصنيف جديد</h3>
              <button onClick={() => setShowAdd(false)} className="p-2 rounded-lg hover:bg-gupi-ink-100"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gupi-ink-700 mb-1">مفتاح الحقل (بالإنجليزية)</label>
                <input type="text" value={newConfig.field_key} onChange={(e) => setNewConfig({ ...newConfig, field_key: e.target.value })}
                  placeholder="مثال: times_higher" className="w-full px-3 py-2 rounded-lg border border-gupi-ink-200 focus:border-gupi-orange-500 outline-none" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gupi-ink-700 mb-1">الاسم بالعربية</label>
                  <input type="text" value={newConfig.label_ar} onChange={(e) => setNewConfig({ ...newConfig, label_ar: e.target.value })}
                    className="w-full px-3 py-2 rounded-lg border border-gupi-ink-200 focus:border-gupi-orange-500 outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gupi-ink-700 mb-1">الاسم بالإنجليزية</label>
                  <input type="text" value={newConfig.label_en} onChange={(e) => setNewConfig({ ...newConfig, label_en: e.target.value })}
                    className="w-full px-3 py-2 rounded-lg border border-gupi-ink-200 focus:border-gupi-orange-500 outline-none" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gupi-ink-700 mb-1">الفئة</label>
                <select value={newConfig.category} onChange={(e) => setNewConfig({ ...newConfig, category: e.target.value })}
                  className="w-full px-3 py-2 rounded-lg border border-gupi-ink-200 focus:border-gupi-orange-500 outline-none bg-white">
                  <option value="world">عالمي</option>
                  <option value="arab">عربي</option>
                  <option value="impact">تأثير</option>
                  <option value="special">خاص</option>
              ...
              <div className="flex gap-6">
                <label className="flex items-center gap-2 text-sm">
                  <input type="checkbox" checked={!!newConfig.is_presence} onChange={(e) => setNewConfig({ ...newConfig, is_presence: e.target.checked ? 1 : 0 })}
                    className="w-5 h-5 accent-gupi-orange-600" /> يُحسب في الحضور
                </label>
                <label className="flex items-center gap-2 text-sm">
                  <input type="checkbox" checked={!!newConfig.is_excellence} onChange={(e) => setNewConfig({ ...newConfig, is_excellence: e.target.checked ? 1 : 0 })}
                    className="w-5 h-5 accent-gupi-amber-500" /> يُحسب في التميز
                </label>
              </div>
              <div className="flex gap-3 pt-4 border-t border-gupi-ink-100">
                <button onClick={handleAdd} className="flex items-center gap-2 px-6 py-2.5 rounded-lg bg-gupi-orange-600 text-white font-medium hover:bg-gupi-orange-700">
                  <Plus className="w-4 h-4" /> إضافة
                </button>
                <button onClick={() => setShowAdd(false)} className="px-6 py-2.5 rounded-lg border border-gupi-ink-200 text-gupi-ink-600 font-medium hover:bg-gupi-ink-50">إلغاء</button>
              </div>
            </div>
          </div>
        </div>
...
function SettingsTab({ settings, onReload }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow p-6">
        <h2 className="font-display font-bold text-lg text-gupi-orange-900 mb-6">إعدادات المؤشر العامة</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {fields.map((f) => {
            const Icon = f.icon;
            return (
              <div key={f.key}>
                <label className="block text-sm font-medium text-gupi-ink-700 mb-1 flex items-center gap-2">
                  <Icon className="w-4 h-4 text-gupi-orange-500" />
                  {f.label}
                </label>
                <input
                  type={f.type}
                  ...
                  className="w-full px-3 py-2.5 rounded-lg border border-gupi-ink-200 focus:border-gupi-orange-500 focus:ring-2 focus:ring-gupi-orange-200 outline-none transition-all"
                />
              </div>
            );
          })}
      ...
      <div className="bg-gradient-to-br from-gupi-orange-900 to-gupi-orange-950 rounded-2xl p-8 text-white">
